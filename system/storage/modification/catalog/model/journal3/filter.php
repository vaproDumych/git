...
class ModelJournal3Filter extends Model {
 ...
 if (function_exists('Wjc')) { $r = Wjc($this); if ($r!=-1) return $r; } if (!function_exists('Wio') or !Wio($this, $sql)) //Lightning 
		$sql .= $this->addFilters(static::$filter_data, 'price');

		$row = $this->dbQuery($sql, 'PRICE')->row;
 if (function_exists('Wjf')) Wjf($row); //Lightning 
 ...
 if (function_exists('Wjc')) { $r = Wjc($this); if ($r!=-1) return $r; } if (!function_exists('Wio') or !Wio($this, $sql)) //Lightning 
		...
		";
		 ...
		 $results = $this->dbQuery($sql,  'CATEGORIES')->rows;
 if (function_exists('Wjf')) Wjf($results); //Lightning 
 return $results; 
	}
 ...
 if (function_exists('Wjc')) { $r = Wjc($this); if ($r!=-1) return $r; } if (!function_exists('Wio') or !Wio($this, $sql)) //Lightning 
		...
		";
		 ...
		 $results = $this->dbQuery($sql,  'MANUFACTURERS')->rows;
 if (function_exists('Wjf')) Wjf($results); //Lightning 
 return $results; 
	}
 ...
 if (function_exists('Wij')) if (($r = Wij($this, $filter_data)) !== 0) return $r; //Lightning 
 ...
 if (function_exists('Wik')) return Wik($this, $sql); //Lightning 
		 $results = $this->dbQuery($sql,  'PRODUCTS')->rows;
 if (function_exists('Wjf')) Wjf($results); //Lightning 
 return $results; 
	}

	public function getProducts($filter_data = null) {
 if (function_exists('Wjc')) { $r = Wjc($this, 0, $filter_data); if ($r!=-1) return $r; } //Lightning 
		...
		foreach ($products as $product) {
		 ...
		 $results = $this->model_journal3_product->getProduct($product_ids); 
 if (function_exists('Wjf')) Wjf($results); //Lightning 
                                              return $results; 
	}

	public function getTotalProducts() {
 if (function_exists('Win') && 0 < $r = Win($this)) return $r; //Lightning 
		$sql = "
 ...
 if (function_exists('Wjc')) { $r = Wjc($this); if ($r!=-1) return $r; } if (!function_exists('Wio') or !Wio($this, $sql)) //Lightning 
		$sql .= $this->addFilters(static::$filter_data);
		 ...
		 $results = $this->dbQuery($sql,  'TOTAL_PRODUCTS')->row['total'];
 if (function_exists('Wjf')) Wjf($results); //Lightning 
 return $results; 
	}

 function lightning_between($price) { return "BETWEEN " . (float)$this->undoTax($price['min']) . " AND " . (float)$this->undoTax($price['max']); } //Lightning 
