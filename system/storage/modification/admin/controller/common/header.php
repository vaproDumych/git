...
class ControllerCommonHeader extends Controller {
	public function index() {
            ...
            $data['dbcache_status'] = $this->config->get((version_compare(VERSION, '3', '>=') ? 'module_' : '').'dbcache_status');
            
            if ($data['dbcache_status']) {
            	$this->language->load('module/dbcache');
            
            	$data['text_dbcache_clear'] = $this->language->get('text_dbcache_clear');
            
            	$url = "";
            
            	if (isset($this->session->data['user_token']) || isset($this->session->data['token'])) {
            		$url .= (version_compare(VERSION, '3', '>=') ? 'user_token='.$this->session->data['user_token'] : 'token='.$this->session->data['token']);
            	}
            	
            	if (isset($_GET['route'])) {
            		$url .= '&dbcache_redirect='.$_GET['route'];
            	}            	
            
				$data['dbcache_clear'] = $this->url->link('module/dbcache/clear', $url, true);
			}
 ...
 if (defined('DIR_CATALOG')) { $data['show_lightning'] = $this->user->hasPermission('modify', 'setting/setting'); if( $data['no_lightning'] = !is_file(DIR_CATALOG.'controller/extension/lightning/gamma.php')) $data['show_lightning'] = 0; } //Lightning 
