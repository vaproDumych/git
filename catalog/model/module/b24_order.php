...
class ModelModuleB24Order extends Model
{
	...
	public function addToDB( $orderId, $b24Id, array $fields = [])
	{
		...
		$fieldsToAdd = ['oc_order_id' => $orderId, 'b24_order_id' => $b24Id];
	...
	public function addOrder( $orderId )
	{
		...
		if( (int) $order['order_status_id'] <= 0 ){ return ;}
...
//endregion Get B24 Product ID
	...
	{
		$b24Contact = $this->getB24ContactList(['EMAIL' => $contactEmail])[0];
	...
	{
		if(empty($filter)){trigger_error('Empty filter', E_USER_WARNING);}
	...
	{
		if(abs($orderId ) <= 0){ trigger_error('Customer ID must be integer', E_USER_WARNING);}
