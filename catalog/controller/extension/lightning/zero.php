...
if($Mz=="s"){$Mve=true;}else if($Mz)require_once"alpha.php";if(!$Mve)if(!$Ma||!is_file(Wa)||filemtime(Wa)<time()-60*60*12){if(!We()){if($Mz){echo("false");$light_ob=false;exit;}
...
if($Ma['gg'])unset($Mq['_']);if($Mz=="s"){echo"<b>Session:</b><br/>";Whr(DIR_IMAGE."cache/session.txt",$Mt);var_dump($Mt);echo"<br/><br/><br/><b>Server:</b><br/>";var_dump($_SERVER);exit;}
...
if($Mr){$Mau=DIR_CACHE."lightning/gamma".$Map;if(file_exists($Mau)){@touch($Mau,$Ml-1);@touch($Man.$Map,$Ml-1);}}
...
function Wfo(){if(lc('hk'))return;$Msq=Wa.'ee';if(file_exists($Msq)&&filemtime($Msq)>time()-60)return;Wfp();global$Mir;if(!file_exists($Msq)){$Mdw=$Mir->query("SELECT date_modified FROM ".DB_PREFIX."product ORDER BY date_modified DESC LIMIT 1")->row["date_modified"];file_put_contents($Msq,$Mdw,LOCK_EX);return;}
...
if(!$Mvp)return$Mvm;imagepalettetotruecolor($Mvp);if($M_m!="image/png"){imagealphablending($Mvp,true);imagesavealpha($Mvp,true);}
Whm($Mvp,$Mvn.$Mvo);imagedestroy($Mvp);return str_replace($Moa,"\/",$Mcj.$Mh.Whp($Mvo));}
