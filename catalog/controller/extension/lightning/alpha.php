...
if($Mcy)$Mds[]=$Mcy;}
$Mda=Wen($Mds);$Mda.='c';if(!$Mpj)$Mda.='s';$Mda=lc('bf').$Mda;if(light_device()){$Mda.="_".light_device();}
if($Md){$Mda.="s";}
if($Mdo){$Mda.="_debug";}
...
if($Mpj){$Md_=str_replace("@font-face{","\n@font-face{font-display:swap;",$Md_);$Md_=str_replace("font-display:swap;font-family:'icomoon","font-family:'icomoon",$Md_);if(!lc('gh'))$Md_=str_replace("font-display:swap;font-family:'FontAwesome","font-family:'FontAwesome",$Md_);}
...
$Meh=true;$Meg=explode(',',$Mee);foreach($Meg as$Mee){$Mee=trim($Mee);$Mee=Wau(":not(*)",'',$Mee);if(!$Mee){continue;}
...
global$Maab;if($Maab)$Mem=Wau("<!--liaj-->*<!--/liaj-->",'',$Mem);Whn($Mem);$Mep=Wa_($Mem);global$Maq;if($Maq){Waz();global$Mtf;Wq(!empty($Maq['Mbz'])or$Mtf);}
...
global$Meo;$Meo[]=$Mrk;$Maaa=empty(W_("request")->get["route"]);if($Maaa){global$Maab;$Maab=true;$Mem="<div id='liaj".count($Meo)."'><!--liaj-->$Mem<!--/liaj--></div>";}
else$Mem="<div id='liaj".count($Meo)."'></div>";array_pop($Mrj);return false;}
...
$Mxb=$Mxc.lc('bf');$Mg=Wbd("alpha",$My.$Mxb);if(!$Mg)return;global$light_ob;$light_ob=0;http_response_code(404);header("X-Lightning-Info: '$My.$Mxb' cached template used");Wz($Mg);exit;}
