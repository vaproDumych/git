...
class ControllerToolNitro extends Controller { 
    ...
    public function index() {
        ...
        $data['pagespeedPrecacheUrl'] = preg_replace("/^https?\:\/\//", "//", html_entity_decode($this->url->link("tool/nitro/precacheForPagespeed", "token=" . $this->session->data["token"])));
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'tool/nitro')) {
            ...
            if ($this->model_tool_nitro->setPersistence($this->request->post)) {
                ...
                $this->model_tool_nitro->applyHtaccessRules();
        ...
        $data['precacheStackUrl'] = HTTP_CATALOG . 'index.php?route=tool/nitro/get_pagecache_stack&cron_token=' . $data['nitroData']['Nitro']['CRON']['Remote']['Token'];
    ...
    public function get_css_extract_stack() {
        ...
        foreach ($stores as $store) {
            ...
            $product = $this->model_tool_nitro->getProductByStoreId($store["store_id"]);
            if ($product) {
                foreach ($user_agents as $strategy => $user_agent) {
                    $pages[] = array(
                        "type" => "product",
            ...
            $category = $this->model_tool_nitro->getCategoryByStoreId($store["store_id"]);
            if ($category) {
                foreach ($user_agents as $strategy => $user_agent) {
                    $pages[] = array(
                        "type" => "category",
