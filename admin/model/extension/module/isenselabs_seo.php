...
class ModelExtensionModuleiSenseLabsSeo extends Model {  
    ...
    public function initDb() {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "seo_url_alias` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
            `query` varchar(255) NOT NULL,
            `keyword` varchar(255) NOT NULL,
            `language_id` int(11) NOT NULL DEFAULT '1',
        ...
        if ($check_if_table_is_filled == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                ...
                (35, 0, 'feed_product_limit', '100'),
                (36, 0, 'feed_category_product', '0'),
                (37, 0, 'feed_manufacturer_product', '0')");
        ...
        $check_if_unify_field_exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='unify_urls'")->num_rows;
        
        if ($check_if_unify_field_exists == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                (`store_id`, `key`, `value`) VALUES
                (0, 'unify_urls', '0'),
                (0, 'breadcrumb_products', '0'),
                (0, 'breadcrumb_categories', '0')");         
        }
        
        $check_if_auto_generate_exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='url_product_autogenerate'")->num_rows;
        
        if ($check_if_auto_generate_exists == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                (`store_id`, `key`, `value`) VALUES
                (0, 'url_product_autogenerate', '0'),
                (0, 'url_category_autogenerate', '0'),
                (0, 'url_manufacturer_autogenerate', '0'),
                (0, 'url_information_autogenerate', '0') ");         
        }
        
        $check_if_subfolders_exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='subfolder_prefixes'")->num_rows;
        
        if ($check_if_subfolders_exists == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                (`store_id`, `key`, `value`) VALUES
                (0, 'subfolder_prefixes', '0')");         
        }
        
        $check_if_richsnippet_category_breadcrumbs_exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='richsnippets_category_breadcrumbs'")->num_rows;
        
        if ($check_if_richsnippet_category_breadcrumbs_exists == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                (`store_id`, `key`, `value`) VALUES
                (0, 'richsnippets_category_breadcrumbs', '0')");         
        }

        $check_custom_url_multi_language = $this->db->query("SELECT COUNT(*) as `count`  FROM `" . DB_PREFIX . "seo_custom_urls`");
        $test_custom_url_multi_language = $this->getCustomUrls();
      
        if ($check_custom_url_multi_language->row['count'] && empty($test_custom_url_multi_language)) {
            $custom_url_multi_language_group = $this->db->query("select * FROM `" . DB_PREFIX . "seo_custom_urls` GROUP BY language_id ORDER BY language_id ASC");
            ...
            foreach ($custom_url_multi_language_clones->rows as $custom_urls) {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_custom_urls` SET `keyword` = '" . $this->db->escape($custom_urls['keyword']) . "', `query` = '" . $this->db->escape($custom_urls['query']). "', `language_id` = '" . (int)$this->config->get('config_language_id') . "'");
            }
        }

        $check_if_sitemap_feed_exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='feed_product_limit'")->num_rows;
        
        if ($check_if_sitemap_feed_exists == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                (`store_id`, `key`, `value`) VALUES
                (0, 'feed_product_limit', '100'),
                (0, 'feed_category_product', '0'),
                (0, 'feed_manufacturer_product', '0') ");
        }

        $check_if_product_heading_tags_info_filled = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='h1_heading_tags_string' AND `store_id` = 0")->num_rows;
        ...
        if ($check_if_product_heading_tags_info_filled == 0) {
            ...
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                ...
                (0, 'h1_heading_tags_string', '[product]'),

                (0, 'h2_heading_tags_string', '[product]')");         

        }

        $check_if_cyrillic_urls_exist = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='cyrillic_urls' AND `store_id` = 0")->num_rows; 
        ...
        $check_if_redirect_to_seo_links_exist = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='redirect_to_seo_links' AND `store_id` = 0")->num_rows;     
        ...
        if ($check_if_redirect_to_seo_links_exist == 0) {
            ...
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_module_settings` 
                ...
                (0, 'redirect_to_seo_links', '0')");  

        }

        $check_if_include_default_lang_prefix_exist = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `key`='default_lang_prefix' AND `store_id` = 0")->num_rows;
    ...
    public function seoUrlCheck() {
        $new_urls = $this->db->query("SELECT count(*) as count FROM `" . DB_PREFIX . "seo_url_alias`")->row['count'];
        $old_urls = $this->db->query("SELECT count(*) as count FROM `" . DB_PREFIX . "url_alias`")->row['count'];
        
        if ($new_urls > 0) {
            return false;
        } else if ($old_urls == 0 && $new_urls == 0) {
            return false;
        } else {
            return true;
        }
    ...
    public function migrateOldSeoUrlsToNewTable() {
        $seo_urls = $this->db->query("SELECT * FROM `" . DB_PREFIX . "url_alias`");
        $use_language_id = false;
        
        $check_lang_column = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "url_alias` LIKE 'language_id'");
		if ($check_lang_column->rows) {
			$use_language_id = true;
		}
        
        $default_language_id = $this->config->get('config_language_id');
        
        foreach ($seo_urls->rows as $seo_url) {
            if ($use_language_id) {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_alias` (query, keyword, language_id) VALUES ('" . $seo_url['query'] . "', '" . $seo_url['keyword'] . "', '" . $seo_url['language_id'] . "')");
            } else {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_alias` (query, keyword, language_id) VALUES ('" . $seo_url['query'] . "', '" . $seo_url['keyword'] . "', '" . $default_language_id . "')");
            }
        }
    ...
    public function getKeywords($item_id = 0, $type = 'product') {
        ...
        switch($type) {
            case "category":
                $results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = 'category_id=" . (int)$this->db->escape($item_id) . "'");
                break;
            case "product":
                $results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = 'product_id=" . (int)$this->db->escape($item_id) . "'");
                break;
            case "manufacturer":
                $results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = 'manufacturer_id=" . (int)$this->db->escape($item_id) . "'");
                break;
            case "information":
                $results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = 'information_id=" . (int)$item_id . "'");
                break;
            default:
                return false;
                break;
    ...
    public function checkSEOUrlsCount() {
        ...
        foreach ($languages as $language) { 
            ...
            $total_products_with_seo_titles[$language['language_id']] = $this->db->query("SELECT count(*) as `count` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%product_id=%' AND `language_id` = " . $language['language_id'] . "")->row['count'];
            ...
            $total_categories_with_seo_titles[$language['language_id']] = $this->db->query("SELECT count(*) as `count` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%category_id=%' AND `language_id` = " . $language['language_id'] . "")->row['count'];
            ...
            $total_manufacturers_with_seo_titles[$language['language_id']] = $this->db->query("SELECT count(*) as `count` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%manufacturer_id=%' AND `language_id` = " . $language['language_id'] . "")->row['count'];
            ...
            $total_informations_with_seo_titles[$language['language_id']] = $this->db->query("SELECT count(*) as `count` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%information_id=%' AND `language_id` = " . $language['language_id'] . "")->row['count'];
    ...
    public function createSEOUrls($type = '', $language_id = '1') {
        ...
        switch($type) {
            case "categories":
                $ids = $this->db->query("SELECT c.category_id FROM `" . DB_PREFIX . "category` c WHERE CONCAT('category_id=',c.category_id) NOT IN (SELECT `query` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = CONCAT('category_id=',c.category_id) AND `language_id`=" . $language_id . ")")->rows;
                
                $result = $this->SeoCategoryURLs($ids, $language_id);
                return $result;
            case "categories_all":
                $this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%category_id=%' AND `language_id`=" . $language_id . "");
                ...
                return $result;
            case "products":
                $ids = $this->db->query("SELECT p.product_id FROM `" . DB_PREFIX . "product` p WHERE CONCAT('product_id=',p.product_id) NOT IN (SELECT `query` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = CONCAT('product_id=',p.product_id) AND `language_id`=" . $language_id . ")")->rows;
                
                $result = $this->SeoProductURLs($ids, $language_id);
                return $result;
            case "products_all":
                $this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%product_id=%' AND `language_id`=" . $language_id . "");
                ...
                return $result;
            case "manufacturers":
                $ids = $this->db->query("SELECT m.manufacturer_id FROM `" . DB_PREFIX . "manufacturer` m WHERE CONCAT('manufacturer_id=',m.manufacturer_id) NOT IN (SELECT `query` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = CONCAT('manufacturer_id=',m.manufacturer_id) AND `language_id`=" . $language_id . ")")->rows;
                
                $result = $this->SeoManufacturerURLs($ids, $language_id);
                return $result;
            case "manufacturers_all":
                $this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%manufacturer_id=%' AND `language_id`=" . $language_id . "");
                ...
                return $result;
            case "informations":
                $ids = $this->db->query("SELECT i.information_id FROM `" . DB_PREFIX . "information` i WHERE CONCAT('information_id=',i.information_id) NOT IN (SELECT `query` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` = CONCAT('information_id=',i.information_id) AND `language_id`=" . $language_id . ")")->rows;
               
                $result = $this->SeoInformationURLs($ids, $language_id);
                return $result;
            case "informations_all":
                $this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_alias` WHERE `query` LIKE '%information_id=%' AND `language_id`=" . $language_id . "");
    ...
    public function fixMinusSEOUrls($type = '', $language_id = '1') {
        ...
        switch ($type) {
            case 'product':
                $items = $this->db->query("SELECT s.id FROM `" . DB_PREFIX . "seo_url_alias` s WHERE s.query LIKE '%product_id=%' and SUBSTRING_INDEX(s.query,'=',-1) NOT IN (SELECT product_id FROM `" . DB_PREFIX . "product`) AND s.language_id = " . (int)$language_id)->rows;
                break;
            case 'category':
                $items = $this->db->query("SELECT s.id FROM `" . DB_PREFIX . "seo_url_alias` s WHERE s.query LIKE '%category_id=%' and SUBSTRING_INDEX(s.query,'=',-1) NOT IN (SELECT category_id FROM `" . DB_PREFIX . "category`) AND s.language_id = " . (int)$language_id)->rows;
                break;
            case 'manufacturer':
                $items = $this->db->query("SELECT s.id FROM `" . DB_PREFIX . "seo_url_alias` s WHERE s.query LIKE '%manufacturer_id=%' and SUBSTRING_INDEX(s.query,'=',-1) NOT IN (SELECT manufacturer_id FROM `" . DB_PREFIX . "manufacturer`) AND s.language_id = " . (int)$language_id)->rows;
                break;
            case 'information':
                $items = $this->db->query("SELECT s.id FROM `" . DB_PREFIX . "seo_url_alias` s WHERE s.query LIKE '%information_id=%' and SUBSTRING_INDEX(s.query,'=',-1) NOT IN (SELECT information_id FROM `" . DB_PREFIX . "information`) AND s.language_id = " . (int)$language_id)->rows;
                break;
        }

        foreach ($items as $item) {
            $ids[] = $item['id'];
            $this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_alias` WHERE `id` = " . (int)$item['id'] . " AND `language_id` = " . (int)$language_id);
        }

        return $ids;
    }
    
    public function SeoProductURLs($product_ids = array(), $language_id) {
        ...
        foreach ($product_ids as $id) {
            ...
            $useCyrillicUrl = preg_match('/[\p{Cyrillic}]/u', $string) && $this->getSetting('cyrillic_urls') ? true : false;
            $generated_slug = $this->formatSEOUrl($string, $useCyrillicUrl);

            $slug_exists = $this->db->query("SELECT `query` FROM `" . DB_PREFIX . "seo_url_alias` WHERE `keyword` = '" . $generated_slug . "'")->num_rows;
            if ($slug_exists > 0) {
                $generated_slug = $generated_slug.'-'.$language_id.'-'.$id['product_id'];
            }

            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_alias` (query, keyword, language_id) VALUES ('product_id=" . $id['product_id'] . "', '" . $generated_slug . "', " . $language_id . ")");
    ...
    public function SeoCategoryURLs($category_ids = array(), $language_id) {
        ...
        foreach ($category_ids as $id) {
            ...
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_alias` (query, keyword, language_id) VALUES ('category_id=" . $id['category_id'] . "', '" . $generated_slug . "', " . $language_id . ")");
    ...
    public function SeoManufacturerURLs($manufacturer_ids = array(), $language_id) {
        ...
        foreach ($manufacturer_ids as $id) {
            ...
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_alias` (query, keyword, language_id) VALUES ('manufacturer_id=" . $id['manufacturer_id'] . "', '" . $generated_slug . "', " . $language_id . ")");
    ...
    public function SeoInformationURLs($information_ids = array(), $language_id) {
        ...
        foreach ($information_ids as $id) {
            ...
            if (!empty($information_info) && !empty($information_info['title'])) {
                ...
                $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_alias` (query, keyword, language_id) VALUES ('information_id=" . $id['information_id'] . "', '" . $generated_slug . "', " . $language_id . ")");
    ...
    public function createProductHeadingTags($type = '', $language_id = '1') {
        ...
        switch($type) {
            ...
            case "product_h1_tags":
                ...
                $result = $this->SeoProductH1Tags($ids, $language_id);
            ...
            case "product_h2_tags":
               ...
               $ids = $this->db->query("SELECT p.product_id FROM `" . DB_PREFIX . "product` p WHERE p.product_id NOT IN (SELECT `product_id` FROM `" . DB_PREFIX . "seo_product_description` WHERE `h2` != '' AND `language_id`=" . $language_id . ")")->rows;
                ...
                $result = $this->SeoProductH2Tags($ids, $language_id);
    ...
    public function SeoProductH1Tags($product_ids = array(), $language_id) {

        $pattern    = $this->getSetting('h1_heading_tags_string');
    ...
    public function SeoProductH2Tags($product_ids = array(), $language_id) {

        $pattern    = $this->getSetting('h2_heading_tags_string');
    ...
    public function checkProductHeadingTagsCount() {
        ...
        foreach ($languages as $language) { 
            ...
            $total_products_with_h1_tags[$language['language_id']] = $this->db->query("SELECT count(*) as `count` FROM `" . DB_PREFIX . "seo_product_description` WHERE `h1` != '' AND `language_id` = " . $language['language_id'] . "")->row['count'];
            ...
            $total_products_with_h2_tags[$language['language_id']] = $this->db->query("SELECT count(*) as `count` FROM `" . DB_PREFIX . "seo_product_description` WHERE `h2` != '' AND `language_id` = " . $language['language_id'] . "")->row['count'];
    ...
    public function getCustomUrls($page = 1, $limit = 10) {
        $query = "SELECT * FROM `" . DB_PREFIX . "seo_custom_urls` WHERE `language_id` = '" . $this->config->get('config_language_id') . "' ";
    ...
    public function getTotalCustomUrls() {
		$query = "SELECT COUNT(*) as `count`  FROM `" . DB_PREFIX . "seo_custom_urls` WHERE `language_id` = '" . $this->config->get('config_language_id') . "'";
    ...
    public function addCustomUrl($data = array()) {
        $this->deleteCustomUrl($data['query']);

        foreach ($data['keyword'] as $language_id => $keyword) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_custom_urls` SET `keyword` = '" . $this->db->escape($keyword) . "', `query` = '" . $this->db->escape($data['query']). "', `language_id` = '" . (int)$language_id . "'");
        }
        
        return true;
    }

    public function getCustomUrl($query) {
        $results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_custom_urls` WHERE `query`= '" . $this->db->escape($query) . "'");
    ...
    public function deleteCustomUrl($query) {
        $query = $this->db->query("DELETE FROM `" . DB_PREFIX . "seo_custom_urls` WHERE `query`= '" . $this->db->escape($query) . "'");
    ...
    public function getDetectedPages($page = 1, $limit = 10, $filter_route = "", $visits="", $date_start="", $date_end="") {
    ...
    public function getTotalDetectedPages($filter_route = "", $visits="", $date_start="", $date_end="") {
    ...
    public function clearMissingPagesResults() {
        $query = $this->db->query("TRUNCATE `" . DB_PREFIX . "seo_404_pages`");
    ...
    public function getRedirects($page = 1, $limit = 10, $route_from = "", $route_to = "") {
