  ...
  protected function optionHandler($field, &$config) {
    ...
    */
    ...
    foreach ($values_array as $current_key => &$value) {
      ...
      $option_names = $option_values = array();
      $option_type = $option_name = $option_price = $option_required = $option_weight = $option_qty = $option_subtract = '';
      ...
      if ($this->filetype == 'xml' && !empty($config['option_fields'])) {
        ...
        if (isset($config['option_fields']['name']) && !empty($value[$config['option_fields']['name']])) {
          $option_names[] = $this->getArrayPath($value, $config['option_fields']['name']);
        }

        if (isset($config['option_fields']['value']) && strpos($config['option_fields']['value'], '+')) {
        ...
        } else if (isset($config['option_fields']['value']) && strpos($config['option_fields']['value'], '|')) {
          $valFields = explode('|', $config['option_fields']['value']);
          $option_values[] = '';
        ...
        } else if (!empty($config['option_fields']['value'])) {
          $option_values[] = $this->getArrayPath($value, $config['option_fields']['value']);
        }
        ...
        foreach($option_values as $i => $option_value) {
          if (empty($option_names[$i])) {
            $optNames = explode('+', $config['option_fields_default']['name']);
            
            $option_names[$i] = isset($optNames[$i]) ? $optNames[$i] : 'Size';
          }
          if (empty($option_value))
            $option_values[$i] = isset($config['option_fields_default']['value']) ? $config['option_fields_default']['value'] : '';
        }
  ...
  */
  ...
  protected function attributeHandler($field, &$config) {
    ...
    foreach ($values_array as $current_key => &$value) {
      ...
      } else if ($this->filetype == 'xml' && is_array($value) && !empty($config['attribute_fields']['value'])) {
        if (!empty($config['attribute_fields']['group'])) {
          $attribute_group_name = $this->getArrayPath($value, $config['attribute_fields']['group']);
        }
        
        if (!empty($config['attribute_fields']['name'])) {
          $attribute_name = $this->getArrayPath($value, $config['attribute_fields']['name']);
        }
        
        if (!empty($config['attribute_fields']['value'])) {
          $attribute_text = $this->getArrayPath($value, $config['attribute_fields']['value']);
        }
        
        // set defaults if empty
        if (empty($attribute_group_name)) {
          $attribute_group_name = isset($config['attribute_fields_default']['group']) ? $config['attribute_fields_default']['group'] : 'Default';
        }
        
        if (empty($attribute_name)) {
          $attribute_name = isset($config['attribute_fields_default']['name']) ? $config['attribute_fields_default']['name'] : '';
        }
        
        if (empty($attribute_text)) {
          $attribute_text = isset($config['attribute_fields_default']['value']) ? $config['attribute_fields_default']['value'] : '';
        }
  ...
  protected function categoryHandler($field, &$config) {
    ...
    foreach ((array) $config['columns'][$field] as $categories) {
      ...
      if (is_string($categories) && !empty($config['col_binding'][md5($categories)])) {
        if ($this->simulation) {
          foreach ($config['col_binding'][md5($categories)] as $colBindId) {
